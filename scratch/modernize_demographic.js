...
const fullDemographicCode = `import { View, Text, TextInput, TouchableOpacity, ScrollView, ActivityIndicator, Alert, Dimensions, Image, Platform, Modal, StyleSheet, Clipboard } from 'react-native';
...
export default function DemographicScreen() {
    ...
    const handleVerify = async () => {
        ...
        if (!cleanFirst || !cleanLast || !cleanDob) {
            return showAlert('Missing Details', 'Please fill in all demographic details to perform lookup.', 'warning');
        }

        setLoading(true);
        try {
            const response = await api.identity.verifyDemographic({
            ...
            if (response.isValid && response.data) {
                ...
                const msg = response.message || 'Unable to find records matching these demographic details.';
                ...
                if (lowerMsg.includes('insufficient') || lowerMsg.includes('balance')) {
                    ...
                    showAlert('No Record Found', 'No record matches the provided details. Please check spelling and date of birth.', 'error');
                } else {
    ...
    const handleDownloadPdf = async () => {
        ...
        try {
            ...
            const genderVal = result.data.gender || result.data.sex || 'N/A';
            const dobVal = formatDob(result.data.birthdate || result.data.dob || 'N/A');
                            ...
                            <div class="grid-main">
                                ...
                                <div class="col-details">
                                    ...
                                    <div class="detail-item" style="margin-top: 4px;">
                                        ...
                                        <div class="detail-val" style="white-space: nowrap;">\${dobVal}</div>
                                    </div>
                                    <div class="detail-item" style="margin-top: 4px;">
                                        <div class="detail-lbl">Gender:</div>
                                        <div class="detail-val">\${genderVal}</div>
                                    </div>
                                </div>
    ...
    const faqs = [
        { q: 'Yaya tsarin binciken yake aiki?', a: 'Zaɓi siffar katin da kake so (Layout), sanya sunaye, jinsi, da ranar haihuwa na mutum, sannan danna Search Demographics. Tsarin zai zaƙulo bayanan mutumin nan take.' },
    ...
    return (
        <View style={styles.container}>
                ...
                <Modal transparent visible={loading} animationType="fade">
                    <View style={styles.loaderOverlay}>
                        <View style={styles.loaderCard}>
                            ...
                            <Text style={styles.loaderTitle}>Searching Demographics</Text>
            ...
            <LinearGradient colors={['#060d21', '#0B163A']} style={[styles.headerGradient, { paddingTop: insets.top > 0 ? insets.top + 10 : 24 }]}>
                <Text style={styles.headerTitle}>Demographic Search</Text>
            </LinearGradient>
            ...
            <ScrollView style={{ flex: 1, paddingHorizontal: 12, marginTop: 12 }} contentContainerStyle={styles.scrollContent}>
                ...
                <View style={styles.card}>
                    <View style={styles.cardHeader}>
                        ...
                        <Text style={styles.cardTitle}>Demographic Details</Text>
                    </View>
                    ...
                    <TouchableOpacity 
                        ...
                        style={[
                            styles.verifyButton,
                            (loading || !firstname.trim() || !lastname.trim() || !dob.trim()) ? styles.verifyButtonDisabled : styles.verifyButtonActive
                        ]}
                        activeOpacity={0.8}
                    >
                        {loading ? <ActivityIndicator color="#f5a623" size="small" /> : (
                            <Text style={styles.verifyButtonText}>SEARCH DEMOGRAPHICS</Text>
                        )}
                    </TouchableOpacity>
                </View>
                ...
                {historyList.length > 0 && (
                    <View style={styles.searchContainer}>
                        ...
                        <TextInput
                            placeholder="Search past lookups (Name or NIN)..."
                            placeholderTextColor="#94a3b8"
                                ...
                                <View key={item.id} style={styles.historyItem}>
                                    ...
                                    >
                                        <View style={styles.historyIconContainer}>
                                            <Ionicons name="person-circle-outline" size={16} color="#060d21" />
                                        </View>
                                        <View style={{ flex: 1 }}>
...
fs.writeFileSync(targetPath, fullDemographicCode, 'utf8');
console.log('Successfully completed full modernization of demographic.tsx!');
