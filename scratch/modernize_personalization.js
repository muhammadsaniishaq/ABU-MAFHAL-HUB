...
if (fs.existsSync(adminPath)) {
    ...
    // Update Tab State type in nin-pricing.tsx to include 'personalization'
    content = content.replace(
    ...
    // Update fetchPrices with personalization seeder and query select
    ...
    // Update Stats Card Row to include personalization pricing stats
    content = content.replace(
    ...
    // Adjust statCard width to fit 5 columns (NIN, IPE, VAL, PERS, AVG)
    content = content.replace(
        `width: '23.5%',`,
        `width: '18.5%',`
    );

    // Update Tab UI to wrap tabs in ScrollView and include Personalization Tab
    ...
    // Update sectionHeaderTitle mapping
    content = content.replace(
    ...
    // Adjust tabContainer styling to allow horizontal scrolling safely
    content = content.replace(
        `    tabContainer: {
        flexDirection: 'row',
    ...
    console.log('Successfully completed nin-pricing.tsx modifications for Personalization category!');
}

// 2. Rewrite app/nin-services/tracking.tsx
const fullTrackingCode = `import { View, Text, TextInput, TouchableOpacity, ScrollView, ActivityIndicator, Alert, Dimensions, Image, Platform, Modal, StyleSheet } from 'react-native';
...
fs.writeFileSync(trackingPath, fullTrackingCode, 'utf8');
console.log('Successfully completed full modernization of tracking.tsx with brand colors, safe area, and matching screenshot design!');
