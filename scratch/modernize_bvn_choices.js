...
const sqlPath = path.join(__dirname, '../supabase/seed_bvn_final.sql');

// 1. Create seed_bvn_final.sql
const sqlContent = `-- Remove all old BVN pricing entries
...
console.log('Successfully created supabase/seed_bvn_final.sql!');

// 2. Update app/manage/nin-pricing.tsx with the 5 default items in the seeder
if (fs.existsSync(adminPath)) {
    let content = fs.readFileSync(adminPath, 'utf8');
    content = content.replace(/\r\n/g, '\n');

    // Replace the defaults block
    const oldDefaults = `            // Auto-seed BVN pricing entries if missing
    ...
    const newDefaults = `            // Auto-seed BVN pricing entries if missing
            const bvnPricingDefaults = [
    ...
    console.log('Successfully updated nin-pricing.tsx with new 5-item defaults.');
}

// 3. Rewrite app/bvn-services.tsx to implement the exact three states
                ...
                <View style={styles.card}>
                    ...
                    {searchType === 'card' ? (
                        <View style={[styles.choiceGrid, { justifyContent: 'center' }]}>
                            <TouchableOpacity 
                                style={[styles.choiceCell, styles.choiceSelected, { width: '50%', minHeight: 110 }]}
                                activeOpacity={0.8}
                            >
                                ...
                                <View style={styles.slipImageBox}>
                                    <Image source={require('../assets/images/premium.png')} style={styles.slipPreviewImage as any} resizeMode="contain" />
                                </View>
...
console.log('Successfully completed full modernization of bvn-services.tsx to exact web layout matching three choices!');
