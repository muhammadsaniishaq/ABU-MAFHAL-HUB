...
const filePath = path.join(__dirname, '../components/RegularSlip.tsx');
...
export const RegularSlip = ({ data }: { data: any }) => {
    ...
    const qrCodeUrl = \`https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=NIN-\${cleanNin}-\${lastName.replace(/\\s+/g, '-')}-\${firstName.replace(/\\s+/g, '-')}&color=000000&margin=1\`;
...
console.log('Successfully updated RegularSlip.tsx with New Edition Card style!');
