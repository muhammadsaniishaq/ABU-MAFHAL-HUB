...
export default function ContentManager() {
    ...
    // Announcement state
    const [activeTab, setActiveTab] = useState<'banners' | 'announcements' | 'settings'>('banners');
            ...
            >
                ...
                <View className="flex-row justify-between items-start mb-2">
                    <View className="w-8 h-8 bg-white/10 rounded-lg items-center justify-center border border-white/20">
                        <Ionicons name={activeTab === 'banners' ? "images" : activeTab === 'announcements' ? "megaphone" : "settings"} size={16} color="#f5a623" />
                    </View>
                    {activeTab === 'banners' ? (
                        <TouchableOpacity 
                ...
                <Text className="text-white text-xl font-black mb-0.5 tracking-tight">
                    {activeTab === 'banners' ? 'Marketing' : activeTab === 'announcements' ? 'Announcements' : 'Settings'}
                </Text>
                <Text className="text-indigo-200 font-medium text-xs opacity-80 mb-3">
                    {activeTab === 'banners' ? 'Manage home screen promotions' : activeTab === 'announcements' ? 'Manage global popups & alerts' : 'App Configuration & Maintenance'}
                </Text>
                ...
                <View className="flex-row justify-between pt-3 border-t border-white/10">
                    <View>
                        ...
                        <Text className="text-white font-black text-lg">{banners.filter(b => b.is_active).length}</Text>
                    </View>
                    <View>
                        <Text className="text-indigo-200 text-[9px] font-bold uppercase tracking-wider mb-0.5">Total</Text>
                        <Text className="text-white font-black text-lg">{banners.length}</Text>
                    </View>
                    <View>
